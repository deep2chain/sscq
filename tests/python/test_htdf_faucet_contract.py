...
def test_contract_sscq_faucet_owner(conftest_args):
    with open('sol/sscq_faucet_sol_HtdfFaucet.abi', 'r') as abifile:
...
# def test_contract_sscq_faucet_getOneHtdf(test_contract_sscq_faucet_deposit):  # also ok
@pytest.mark.usefixtures("test_contract_sscq_faucet_deposit")
def test_contract_sscq_faucet_getOneHtdf(conftest_args):
    """
    ...
    for n in range(2):
        ...
        deposit_tx = hc.functions.getOneHtdf().buildTransaction_sscq()
        data = remove_0x_prefix(deposit_tx['data'])
...
def test_contract_sscq_faucet_setOnceAmount(conftest_args):
    ...
    if from_acc is None or from_acc.balance_satoshi < 100 * 200000:
        ...
        sscqrpc = HtdfRPC(chaid_id=conftest_args['CHAINID'], rpc_host=conftest_args['RPC_HOST'],
